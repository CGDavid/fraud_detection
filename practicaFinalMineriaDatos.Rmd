...
Obtenemos un F1-score de 0.65


### Ejemplo 2: 
```{r}
train_smote <- SMOTE(as.data.frame(X_train), as.data.frame(Y_train), K=2, dup_size=5)
train_smote$data$class <- as.integer(train_smote$data$class)

model <- glm(class~., data = train_smote$data, family = binomial())
Y_test_pred <- round(predict(model, test, type = "response"))
...
K: 2, 5, 10
dup_size: 1, 3, 5, 10, 20, 100, 500

La mejor combinación es k=2 y dup_size=5 con un F1_Score de 0.78

### Ejemplo 2: Undersampling + Regresión logística
...
```
Vamos a echar un vistazo a la curva ROC
